...
const LoginForm = () => {
    ...
    function closingLoginForm(){
        globalCon.setLoginFormState(false);
    ...
    useEffect(()=>{
        if (globalCon.loginCheck===true){
            setLoginSuccess(true);
        }
        console.log(globalCon.loginCheck);
    },[globalCon.loginCheck])
    function checkUserInfo(){
        for(let item of globalCon.usersList){
            console.log(item);
            let found;
            if(loginName===item.email){
                if(loginPass===item.pass){
                    globalCon.setLoggedInUser(item);
                    globalCon.setLoginCheck(true);
                    found=true;
                    setErrMsg("");
                }else{
                    found="wrong";
                    ...
                    <div className="row login-form-options">
                        <p>حساب کاربری ندارید؟
                            <span onMouseDown={function(){
                                closingLoginForm();
                                globalCon.setRegisterFormState(true);
